...
//for vTaskGetRunTimeStats in Supervisor
//#include "timer.h"
	...
	if( pxNewTCB != NULL )
	{
		...
		taskENTER_CRITICAL();
		{
			...
			#if ( configUSE_TRACE_FACILITY == 1 )
			{
				...
				pxNewTCB->uxStackSize = usStackDepth;
			}
	...
	unsigned portBASE_TYPE uxTask = 0;
		...
		if( listCURRENT_LIST_LENGTH( pxList ) > ( unsigned portBASE_TYPE ) 0 )
		{
			...
			do
			{
				...
				pxTaskStatusArray[ uxTask ].usStackSize = pxNextTCB->uxStackSize * sizeof( portBASE_TYPE );
...
/*-----------------------------------------------------------*/
	...
	void vTaskList( signed char *pcWriteBuffer, size_t bfr_size )
	{
	xTaskStatusType *pxTaskStatusArray;
	volatile unsigned portBASE_TYPE uxArraySize, x;
	char cStatus;
	unsigned int uiTotalStackSize = 0;
	signed char *bfr = pcWriteBuffer;
		...
		*pcWriteBuffer = 0x00;
		...
		if( pxTaskStatusArray != NULL )
		{
			...
			unsigned int HiWtrFreeStkBytes;
			unsigned int HiWtrUsedStkBytes;
			unsigned int StkSize;
			unsigned int HiWtrPct;
			...
			{
			    task_name_len = strlen( pxTaskStatusArray[ x ].pcTaskName );
			...
			if( max_task_name_len >= 8 )
			{
				spacer = "\t\t";
			}
			else if( max_task_name_len >= 16 )
			{
				spacer = "\t\t\t";
			}
			...
			sprintf( pcWriteBuffer, "All stack & heap sizes in bytes\r\n\r\n" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "Task" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, spacer );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "State\tPrior\tStk Hi\tStk Siz\tStkUsed\tTask#\r\n" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "----" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, spacer );
			...
			/* Now fill in the table with data */
			for( x = 0; x < uxArraySize; x++ )
			{
				...
				if( strlen( pxTaskStatusArray[ x ].pcTaskName ) < 8 )
				{
					spacer = "\t\t";
				}
				else if( strlen( pxTaskStatusArray[ x ].pcTaskName ) >= 16 )
				{
					spacer = "";
				}
				else
				...
				StkSize           =	( unsigned int ) pxTaskStatusArray[ x ].usStackSize;
				HiWtrFreeStkBytes = ( unsigned int ) pxTaskStatusArray[ x ].usStackHighWaterMark;
				HiWtrUsedStkBytes = StkSize - HiWtrFreeStkBytes;
				HiWtrPct 		  = ( 100 * HiWtrUsedStkBytes + ( StkSize / 2) ) / StkSize;
				...
				uiTotalStackSize += StkSize;
			}

			sprintf( pcWriteBuffer, "\r\n" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "Total stack size: %u\r\n\r\n", uiTotalStackSize );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "Heap Statistics\r\n" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "---------------\r\n" );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "Total heap size: %u (%uK)\r\n",
					 configTOTAL_HEAP_SIZE, configTOTAL_HEAP_SIZE / 1024 );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "High water mark: %u (%u%%)\r\n",
					 configTOTAL_HEAP_SIZE - xPortGetMinHeapSize(),
					 ( 100 *  ( configTOTAL_HEAP_SIZE - xPortGetMinHeapSize() ) +
					 ( configTOTAL_HEAP_SIZE / 2 ) ) / configTOTAL_HEAP_SIZE );
			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "Current usage:   %u (%u%%)\r\n",
					 configTOTAL_HEAP_SIZE - xPortGetFreeHeapSize(),
					 ( 100 *  ( configTOTAL_HEAP_SIZE - xPortGetFreeHeapSize() ) +
					 ( configTOTAL_HEAP_SIZE / 2 ) ) / configTOTAL_HEAP_SIZE );

			pcWriteBuffer += strlen( pcWriteBuffer );
			sprintf( pcWriteBuffer, "\r\n" );

			configASSERT( bfr_size > strlen( bfr ) );

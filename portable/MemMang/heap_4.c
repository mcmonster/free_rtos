...
#ifdef CREATE_HEAP_IN_CCM
	/* Allocate the memory for the heap in CCM RAM. */
	static unsigned char ucHeap[ configTOTAL_HEAP_SIZE ] __attribute__ ((section (".ccmram")));
#else
...
static size_t xFreeBytesRemaining = ( ( size_t ) heapADJUSTED_HEAP_SIZE ) & ( ( size_t ) ~portBYTE_ALIGNMENT_MASK );
	...
	static size_t xFreeBytesRemainingMin;
		...
		if( ( xWantedSize & xBlockAllocatedBit ) == 0 )
		{
			...
			if( ( xWantedSize > 0 ) && ( xWantedSize <= xFreeBytesRemaining ) )
			{
				...
				if( pxBlock != pxEnd )
				{
					...
					#if ( configUSE_TRACE_FACILITY == 1 )
						if( xFreeBytesRemaining < xFreeBytesRemainingMin )
						{
							xFreeBytesRemainingMin = xFreeBytesRemaining;
						}
					#endif /* configUSE_TRACE_FACILITY */
...
#if ( configUSE_TRACE_FACILITY == 1 )
size_t xPortGetMinHeapSize( void )
{
	return xFreeBytesRemainingMin;
}
#endif /* configUSE_TRACE_FACILITY */
